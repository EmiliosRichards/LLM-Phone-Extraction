...
def download_image(url: str, path: Path, raise_on_failure: bool = False) -> bool:
    ...
    Returns:
        bool: True if download was successful, False otherwise (unless raise_on_failure is True)
    ...
    for attempt in range(config.IMAGE_RETRY_COUNT):
        try:
            ...
            logging.info(f"Downloading image from {url} (attempt {attempt + 1}/{config.IMAGE_RETRY_COUNT})")
            res = requests.get(url, timeout=config.IMAGE_DOWNLOAD_TIMEOUT)
            
            if res.status_code == 200:
                with open(str(path), 'wb') as f:
                    f.write(res.content)
                logging.info(f"Successfully downloaded image to {path}")
                return True
            else:
                logging.warning(f"Failed to download image (HTTP {res.status_code}): {url}")
                if attempt < config.IMAGE_RETRY_COUNT - 1:
                    logging.info(f"Retrying in {config.IMAGE_RETRY_DELAY} seconds...")
                    time.sleep(config.IMAGE_RETRY_DELAY)
                    continue
                if raise_on_failure:
                    error_msg = f"Failed to download image after {config.IMAGE_RETRY_COUNT} attempts: {url}"
                    logging.error(error_msg)
                    raise RuntimeError(error_msg)
                return False
                
        except (requests.exceptions.Timeout, requests.exceptions.RequestException) as e:
            if handle_download_error(e, url, attempt, raise_on_failure):
                continue
            return False
...
def normalize_hostname(url: str) -> str:
    """Normalize a hostname by removing 'www.' prefix and converting to a safe filename.
    
    Args:
        url (str): URL or hostname to normalize
        
    Returns:
        str: Normalized hostname with dots replaced by underscores and www. removed
    """
